...
export const MatchContextInteraction: React.FC<MatchContextInteractionProps> = ({
  ...
  return (
    <div className="absolute inset-0 flex flex-col h-full z-10 overflow-hidden animate-fade-in pb-safe select-none">
      ...
      {/* 1. Top Section: Bubble + Peter with Toothbrush Together (Approved grounded position & scale) */}
      ...
      {/* 2. Room Choices: 2 Large 1:1 Square Cards Stacked Vertically - Shifted Up into Carpet Area */}
      <div className="flex-1 w-full max-w-sm mx-auto flex flex-col justify-start items-center px-4 -mt-3 sm:-mt-2 pb-6 gap-2.5 sm:gap-3 z-20">
        {choices.map((choice) => (
          <button
            key={choice.id}
